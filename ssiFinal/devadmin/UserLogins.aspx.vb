...
Namespace ssiFinal
	Public Class ActiveUsers
		...
		Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
			If (MyBase.Request("ID") IsNot Nothing) Then
				Me.SqlDataSourceUserLogin.FilterExpression = String.Concat("Id = '", MyBase.Request("ID"), "'")
				Me.GridView1.SelectedIndex = 0
				Me.Session("ID") = MyBase.Request("ID")
				Me.SqlDataSourceUserLogin.Update()
			End If
			Try
				Me.Alerts()
				Me.messages()
		...
		Protected Sub RefreshButton_Click(ByVal sender As Object, ByVal e As EventArgs)
			Try
				Me.SqlDataSourceUserLogin.FilterExpression = Nothing
		...
		Protected Sub SearchButton_Click(ByVal sender As Object, ByVal e As EventArgs)
			Try
				If (Me.SearchDropDownList.SelectedIndex = 0) Then
					Me.SqlDataSourceUserLogin.FilterExpression = String.Concat("User Like '%", Me.SearchTextBox.Text.Trim(), "%'")
				End If
				Me.SqlDataSourceUserLogin.[Select](DataSourceSelectArguments.Empty)
				Me.ViewState.Add("filter", Me.SqlDataSourceUserLogin.FilterExpression)
			Catch exception As System.Exception
				ProjectData.SetProjectError(exception)
				ProjectData.ClearProjectError()
			End Try
