...
Namespace ssiFinal
	Public Class ProcessedRepickupTransactions
        ...
        Private _ButtonDownloadStockReceiptExcel As Button
		...
		Protected Overridable Property ButtonDownloadStockReceiptExcel As Button
			Get
				Return Me._ButtonDownloadStockReceiptExcel
			End Get
			<MethodImpl(MethodImplOptions.Synchronized)>
			Set(ByVal value As System.Web.UI.WebControls.Button)
				Dim eventHandler As System.EventHandler = New System.EventHandler(AddressOf Me.ButtonDownloadStockReceiptExcel_Click)
				Dim button As System.Web.UI.WebControls.Button = Me._ButtonDownloadStockReceiptExcel
				If (button IsNot Nothing) Then
					RemoveHandler button.Click,  eventHandler
				End If
				Me._ButtonDownloadStockReceiptExcel = value
				button = Me._ButtonDownloadStockReceiptExcel
				If (button IsNot Nothing) Then
					AddHandler button.Click,  eventHandler
				End If
			End Set
		...
		Protected Overridable Property SqlDataSourceTransactionRepickupGrid As SqlDataSource

		Protected Overridable Property SqlDataSourcetransactionRepickupItems As SqlDataSource
					...
					Using streamWriter As System.IO.StreamWriter = File.CreateText(String.Concat(MyBase.Server.MapPath("CSV"), "/BarCode.txt"))
						Try
							...
							While enumerator.MoveNext()
								...
								streamWriter.WriteLine(HttpUtility.HtmlDecode(current.Cells(3).Text))
							End While
						Finally
		...
		Protected Sub Button2_Click(ByVal sender As Object, ByVal e As EventArgs)
			...
			Try
				...
				str = String.Concat("StockReceipt_", Conversions.ToString(random.[Next](0, 1000000)), ".pdf")
			...
			Using memoryStream As System.IO.MemoryStream = New System.IO.MemoryStream()
				...
				Try
					...
					While enumerator.MoveNext()
						...
						pdfPCell = ProcessedRepickupTransactions.PhraseCell(phrase, 8)
						...
						pdfPCell = ProcessedRepickupTransactions.PhraseCell(phrase1, 8)
						pdfPTable.AddCell(pdfPCell)
						...
						pdfPCell = ProcessedRepickupTransactions.PhraseCell(phrase2, 0)
						pdfPTable.AddCell(pdfPCell)
						...
						pdfPCell = ProcessedRepickupTransactions.PhraseCell(phrase3, 0)
						pdfPTable.AddCell(pdfPCell)
		...
		Protected Sub Button4_Click(ByVal sender As Object, ByVal e As EventArgs)
			...
			Using memoryStream As System.IO.MemoryStream = New System.IO.MemoryStream()
				...
				MyBase.Response.ContentType = "application/pdf"
				MyBase.Response.AddHeader("content-disposition", "attachment; filename=QRCode.pdf")
				MyBase.Response.BinaryWrite(array)
		...
		Protected Sub ButtonDownloadStockReceiptExcel_Click(ByVal sender As Object, ByVal e As EventArgs)
			If (Me.TransactionPickupItemsGridView.Rows.Count = 0) Then
				Me.renderErrMsgs("Please select a transaction below.")
				Return
			End If
			Try
				Dim data As DataSet1 = Me.GetData(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject("SELECT BarCode, BoxNumber, Description, Retention FROM Repickup WHERE TransactionHeaders_id = '", Me.Session("StockID")), "' AND Status = 'PROCESSED'")))
				...
				reportDocument.Load(MyBase.Server.MapPath("~/devadmin/ExportSTRRepickup.rpt"))
		...
		Protected Sub ButtonSearch_Click(ByVal sender As Object, ByVal e As EventArgs)
			Try
				If (Me.DropDownListSearch.SelectedIndex = 0) Then
					Me.SqlDataSourcetransactionRepickupItems.FilterExpression = String.Concat("BarCode LIKE '%", Me.TextBoxSearch.Text.Trim(), "%'")
				ElseIf (Me.DropDownListSearch.SelectedIndex = 1) Then
					Me.SqlDataSourcetransactionRepickupItems.FilterExpression = String.Concat("Description LIKE '%", Me.TextBoxSearch.Text.Trim(), "%'")
				ElseIf (Me.DropDownListSearch.SelectedIndex = 2) Then
					Me.SqlDataSourcetransactionRepickupItems.FilterExpression = String.Concat("BoxNumber = '", Me.TextBoxSearch.Text.Trim(), "'")
				ElseIf (Me.DropDownListSearch.SelectedIndex = 3) Then
					Me.SqlDataSourcetransactionRepickupItems.FilterExpression = String.Concat("Department = '", Me.TextBoxSearch.Text.Trim(), "'")
				End If
				Me.SqlDataSourcetransactionRepickupItems.[Select](DataSourceSelectArguments.Empty)
			Catch exception As System.Exception
				ProjectData.SetProjectError(exception)
				ProjectData.ClearProjectError()
			End Try
			...
			Using sqlConnection As System.Data.SqlClient.SqlConnection = New System.Data.SqlClient.SqlConnection(connectionString)
				Using sqlDataAdapter As System.Data.SqlClient.SqlDataAdapter = New System.Data.SqlClient.SqlDataAdapter()
					...
					Using dataSet11 As ssiFinal.DataSet1 = New ssiFinal.DataSet1()
						sqlDataAdapter.Fill(dataSet11, "ProcessedRepickupTransactionsItems")
						dataSet1 = dataSet11
					End Using
				End Using
			End Using
		...
		Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
			...
			If (Not Information.IsNothing(MyBase.Request("ID"))) Then
				Me.SqlDataSourceTransactionRepickupGrid.FilterExpression = String.Concat("Id = '", MyBase.Request("ID"), "'")
		...
		Protected Sub RefreshButton_Click(ByVal sender As Object, ByVal e As EventArgs)
			MyBase.Response.Redirect("ProcessedRepickupTransactions")
			Try
		...
		Public Sub renderErrMsgs(ByVal thisErrMsg As String)
			' 
			' Current member / type: System.Void ssiFinal.ProcessedRepickupTransactions::renderErrMsgs(System.String)
		...
		Protected Sub SearchButton_Click(ByVal sender As Object, ByVal e As EventArgs)
			Try
				If (Me.SearchDropDownList.SelectedIndex = 0) Then
					Me.SqlDataSourceTransactionRepickupGrid.FilterExpression = String.Concat("DepartmentCode LIKE '%", Me.SearchTextbox.Text.Trim(), "%'")
				ElseIf (Me.SearchDropDownList.SelectedIndex = 1) Then
					Me.SqlDataSourceTransactionRepickupGrid.FilterExpression = String.Concat("ContactName LIKE '%", Me.SearchTextbox.Text.Trim(), "%'")
				ElseIf (Me.SearchDropDownList.SelectedIndex = 2) Then
					Me.SqlDataSourceTransactionRepickupGrid.FilterExpression = String.Concat("Id = '", Me.SearchTextbox.Text.Trim(), "'")
				ElseIf (Me.SearchDropDownList.SelectedIndex = 3) Then
					Me.SqlDataSourceTransactionRepickupGrid.FilterExpression = String.Concat("RequestDate = '", Me.SearchTextbox.Text.Trim(), "'")
				End If
				Me.SqlDataSourceTransactionRepickupGrid.[Select](DataSourceSelectArguments.Empty)
			Catch exception As System.Exception
				ProjectData.SetProjectError(exception)
				Me.renderErrMsgs(exception.Message.ToString())
				ProjectData.ClearProjectError()
