...
Namespace ssiFinal
	Public Class TransactionPickups
		Inherits System.Web.UI.Page
		Private barCodeDocument As Document

		Public errMsg As String

		Public nl As List(Of TransactionPickups.ssiItem)
		...
		Protected Overridable Property CancelButton As Button
			Get
				Return Me._CancelButton
			End Get
			<MethodImpl(MethodImplOptions.Synchronized)>
			Set(ByVal value As System.Web.UI.WebControls.Button)
				Dim eventHandler As System.EventHandler = New System.EventHandler(AddressOf Me.CancelButton_Click)
				Dim button As System.Web.UI.WebControls.Button = Me._CancelButton
				If (button IsNot Nothing) Then
					RemoveHandler button.Click,  eventHandler
				End If
				Me._CancelButton = value
				button = Me._CancelButton
				If (button IsNot Nothing) Then
					AddHandler button.Click,  eventHandler
				End If
			End Set
		...
		Protected Overridable Property ReceiveSqlDataSource As SqlDataSource
			...
			Set(ByVal value As System.Web.UI.WebControls.SqlDataSource)
				Dim sqlDataSourceStatusEventHandler As System.Web.UI.WebControls.SqlDataSourceStatusEventHandler = New System.Web.UI.WebControls.SqlDataSourceStatusEventHandler(AddressOf Me.ReceiveSqlDataSource_Inserted)
		...
		Protected Overridable Property SqlDataSourcePickupCount As SqlDataSource

		Protected Overridable Property SqlDataSourceReadUserDetails As SqlDataSource

		Protected Overridable Property SqlDataSourceTransactionpickupDetails As SqlDataSource
        ...
        Public Sub New()
            ...
            Me.barCodeDocument = New Document(PageSize.A7.Rotate(), 10.0!, 10.0!, 50.0!, 0!)
			...
			Using memoryStream As System.IO.MemoryStream = New System.IO.MemoryStream()
				...
				Try
					...
					While enumerator.MoveNext()
						...
						pdfPCell = TransactionPickups.PhraseCell(phrase, 8)
						...
						pdfPCell = TransactionPickups.PhraseCell(phrase1, 8)
						pdfPTable.AddCell(pdfPCell)
						Dim phrase2 As iTextSharp.text.Phrase = New iTextSharp.text.Phrase()
						phrase2.Add(New Chunk(String.Concat("Box No: ", DirectCast(current.FindControl("BoxNumberLbl"), Label).Text, "" & VbCrLf & ""), FontFactory.GetFont("Arial", 10!, 1, BaseColor.BLACK)))
						pdfPCell = TransactionPickups.PhraseCell(phrase2, 0)
						pdfPTable.AddCell(pdfPCell)
						Dim phrase3 As iTextSharp.text.Phrase = New iTextSharp.text.Phrase()
						phrase3.Add(New Chunk(String.Concat("Description: ", DirectCast(current.FindControl("DescriptionLbl"), Label).Text), FontFactory.GetFont("Arial", 10!, 1, BaseColor.BLACK)))
						pdfPCell = TransactionPickups.PhraseCell(phrase3, 0)
						pdfPTable.AddCell(pdfPCell)
			...
			Try
				...
				Try
					...
					While enumerator.MoveNext()
						...
						pdfPTable.AddCell(New iTextSharp.text.Phrase(DirectCast(current.FindControl("DepartmentLbl"), Label).Text, FontFactory.GetFont("Calibri", 8!, 0)))
						pdfPTable.AddCell(New iTextSharp.text.Phrase(DirectCast(current.FindControl("BoxNumberLbl"), Label).Text, FontFactory.GetFont("Calibri", 8!, 0)))
						pdfPTable.AddCell(New iTextSharp.text.Phrase(DirectCast(current.FindControl("DescriptionLbl"), Label).Text, FontFactory.GetFont("Calibri", 8!, 0)))
						pdfPTable.AddCell(New iTextSharp.text.Phrase(DirectCast(current.FindControl("RetentionLbl"), Label).Text, FontFactory.GetFont("Calibri", 8!, 0)))
				...
				Try
					...
					While enumerator1.MoveNext()
						...
						pdfPTable.AddCell(New iTextSharp.text.Phrase(DirectCast(gridViewRow.FindControl("DepartmentLbl"), Label).Text, FontFactory.GetFont("Calibri", 8!, 0)))
						pdfPTable.AddCell(New iTextSharp.text.Phrase(DirectCast(gridViewRow.FindControl("BoxNumberLbl"), Label).Text, FontFactory.GetFont("Calibri", 8!, 0)))
						pdfPTable.AddCell(New iTextSharp.text.Phrase(DirectCast(gridViewRow.FindControl("DescriptionLbl"), Label).Text, FontFactory.GetFont("Calibri", 8!, 0)))
						pdfPTable.AddCell(New iTextSharp.text.Phrase(DirectCast(gridViewRow.FindControl("RetentionLbl"), Label).Text, FontFactory.GetFont("Calibri", 8!, 0)))
						pdfPTable.AddCell(New iTextSharp.text.Phrase("Yes___ No___", FontFactory.GetFont("Calibri", 8!, 0)))
						If (pdfPTable.TotalHeight < 350!) Then
							Continue While
						End If
						...
						phrase.Add(New Chunk(String.Concat("" & VbCrLf & "Document No: ", HttpUtility.HtmlDecode(Me.TransactionPickupDetailsView.Rows(0).Cells(1).Text), "" & VbCrLf & ""), FontFactory.GetFont("Calibri", 10!, 0)))
						...
						phrase1.Add(New Chunk(String.Concat("" & VbCrLf & "" & VbCrLf & "Requested: ", HttpUtility.HtmlDecode(Me.TransactionPickupDetailsView.Rows(5).Cells(1).Text), "" & VbCrLf & "" & VbCrLf & ""), FontFactory.GetFont("Calibri", 10!, 1)))
						phrase1.Add(New Chunk(String.Concat("Customer: ", HttpUtility.HtmlDecode(Me.TransactionPickupDetailsView.Rows(19).Cells(1).Text), "" & VbCrLf & "" & VbCrLf & ""), FontFactory.GetFont("Calibri", 10!, 1)))
						phrase1.Add(New Chunk(String.Concat("Contact Name: ", HttpUtility.HtmlDecode(Me.TransactionPickupDetailsView.Rows(1).Cells(1).Text), "" & VbCrLf & "" & VbCrLf & ""), FontFactory.GetFont("Calibri", 10!, 1)))
						phrase1.Add(New Chunk(String.Concat(New String() { "Contact Details: ", HttpUtility.HtmlDecode(Me.TransactionPickupDetailsView.Rows(3).Cells(1).Text), "/", Me.TransactionPickupDetailsView.Rows(7).Cells(1).Text, "" & VbCrLf & "" & VbCrLf & "" }), FontFactory.GetFont("Calibri", 10!, 1)))
						phrase1.Add(New Chunk(String.Concat("Rush: ", HttpUtility.HtmlDecode(Me.TransactionPickupDetailsView.Rows(10).Cells(1).Text)), FontFactory.GetFont("Calibri", 10!, 1)))
		...
		Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
			Try
				Me.nl = New List(Of TransactionPickups.ssiItem)()
				Me.Alerts()
				Me.messages()
				If (Operators.CompareString(MyBase.Request("Notif"), "true", False) = 0) Then
					Me.Session("Type") = "PICKUP"
		...
		Protected Sub ReceiveButton_Click(ByVal sender As Object, ByVal e As EventArgs)
			...
			Try
				...
				While enumerator.MoveNext()
					Dim current As System.Web.UI.WebControls.GridViewRow = DirectCast(enumerator.Current, System.Web.UI.WebControls.GridViewRow)
					Dim checkBox As System.Web.UI.WebControls.CheckBox = TryCast(current.Cells(0).FindControl("ValidationCheckBox"), System.Web.UI.WebControls.CheckBox)
			...
			Finally
				...
				End If
			End Try
			If (Me.TransactionPickupItemsGridView.Rows.Count = 0 Or num = 0) Then
				Me.renderErrMsgs("There are no selected items in Pick Up.")
				Return
			End If
			...
			Try
				...
				While enumerator1.MoveNext()
					Dim gridViewRow As System.Web.UI.WebControls.GridViewRow = DirectCast(enumerator1.Current, System.Web.UI.WebControls.GridViewRow)
					Dim checkBox1 As System.Web.UI.WebControls.CheckBox = TryCast(gridViewRow.Cells(1).FindControl("ValidationCheckBox"), System.Web.UI.WebControls.CheckBox)
					gridViewRow.Cells(10).FindControl("LocationCodeTextBox")
					If (Not checkBox1.Checked) Then
						Continue While
					End If
					Me.ReceiveSqlDataSource.InsertParameters("Department").DefaultValue = HttpUtility.HtmlDecode(DirectCast(gridViewRow.FindControl("DepartmentLbl"), Label).Text)
					Me.ReceiveSqlDataSource.InsertParameters("BarCode").DefaultValue = HttpUtility.HtmlDecode(DirectCast(gridViewRow.FindControl("BarCodeLbl"), Label).Text)
					Me.Session("_Barcode") = HttpUtility.HtmlDecode(DirectCast(gridViewRow.FindControl("BarCodeLbl"), Label).Text)
					Me.ReceiveSqlDataSource.InsertParameters("QRCode").DefaultValue = HttpUtility.HtmlDecode(DirectCast(gridViewRow.FindControl("QRCodeLbl"), Label).Text)
					Me.ReceiveSqlDataSource.InsertParameters("BoxNumber").DefaultValue = HttpUtility.HtmlDecode(DirectCast(gridViewRow.FindControl("BoxNumberLbl"), Label).Text)
					Me.ReceiveSqlDataSource.InsertParameters("Description").DefaultValue = HttpUtility.HtmlDecode(DirectCast(gridViewRow.FindControl("DescriptionLbl"), Label).Text)
					Me.ReceiveSqlDataSource.InsertParameters("DestructionDate").DefaultValue = HttpUtility.HtmlDecode(DirectCast(gridViewRow.FindControl("RetentionLbl"), Label).Text)
					Me.ReceiveSqlDataSource.InsertParameters("Status").DefaultValue = "IN"
					Me.ReceiveSqlDataSource.InsertParameters("LocationCode").DefaultValue = ""
					Me.ReceiveSqlDataSource.InsertParameters("StockReceipt_id").DefaultValue = text
					Me.ReceiveSqlDataSource.InsertParameters("CompanyCode").DefaultValue = Convert.ToString(Me.TransactionPickupDetailsView.Rows(20).Cells(1).Text)
					Me.ReceiveSqlDataSource.InsertParameters("CreatedBy").DefaultValue = MyBase.User.Identity.GetUserName()
					Dim item As System.Web.UI.WebControls.Parameter = Me.ReceiveSqlDataSource.InsertParameters("CreatedDate")
					utcNow = DateTime.UtcNow
					item.DefaultValue = Conversions.ToString(utcNow.AddHours(8))
					Me.ReceiveSqlDataSource.Insert()
					Me.SqlDataSourceUpdatePickupItems.UpdateParameters("Status").DefaultValue = "PROCESSED"
					Dim str As System.Web.UI.WebControls.Parameter = Me.SqlDataSourceUpdatePickupItems.UpdateParameters("ModifiedDate")
					utcNow = DateTime.UtcNow
			...
			Dim parameter As System.Web.UI.WebControls.Parameter = Me.SqlDataSourceUpdateTransactionPickup.UpdateParameters("ModifiedDate")
			utcNow = DateTime.UtcNow
			...
			Me.SqlDataSourceUpdateTransactionPickup.UpdateParameters("Id").DefaultValue = Me.TransactionPickupsGridView.SelectedRow.Cells(1).Text
			Me.SqlDataSourceUpdateTransactionPickup.Update()
			MyBase.Response.Redirect(String.Concat("ProcessedPickupTransactions?ID=", text))
		End Sub

		Private Sub ReceiveSqlDataSource_Inserted(ByVal sender As Object, ByVal e As SqlDataSourceStatusEventArgs)
			Try
				Me.Session("HistoryMasterListId") = RuntimeHelpers.GetObjectValue(e.Command.Parameters("@masterID").Value)
				Me.Session("HistoryDesc") = "PICK UP"
		...
		Public Sub renderErrMsgs(ByVal thisErrMsg As String)
			' 
			' Current member / type: System.Void ssiFinal.TransactionPickups::renderErrMsgs(System.String)
		...
		Private Sub SqlDataSourceTransactionPickupItems_Deleted(ByVal sender As Object, ByVal e As SqlDataSourceStatusEventArgs)
			...
			MyBase.Response.Redirect("~/devadmin/TransactionPickups.aspx", False)
		End Sub
			...
			Try
				...
				Else
					...
					If (Me.Session("tableBackUp") IsNot Nothing) Then
						Me.nl = DirectCast(Me.Session("tableBackUp"), List(Of TransactionPickups.ssiItem))
					End If
