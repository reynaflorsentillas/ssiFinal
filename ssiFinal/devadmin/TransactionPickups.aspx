<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/devadmin/MasterPage.Master" CodeBehind="TransactionPickups.aspx.vb" Inherits="ssiFinal.TransactionPickups" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
                ...
                <div class="widget-body">
                    <div class="control-group">
                        <div class="table-responsive">
                            <asp:DetailsView ID="TransactionPickupDetailsView" runat="server" AutoGenerateRows="False" DataSourceID="SqlDataSourceTransactionpickupDetails" Height="50px" Width="329px" CssClass="table table-striped table-bordered table-advance table-hover" DataKeyNames="Id,Expr1">
                                <Fields>
                            ...
                            <asp:GridView ID="TransactionPickupItemsGridView" runat="server" AutoGenerateColumns="False" DataKeyNames="Id" DataSourceID="SqlDataSourceTransactionPickupItems" CssClass="table table-striped table-bordered table-advance table-hover" AllowSorting="True">
                                <Columns>
                                    ...
                                    <asp:TemplateField HeaderText="Barcode" SortExpression="BarCode">
                                        ...
                                        <ItemTemplate>
                                            <asp:Label ID="BarCodeLbl" Visible="<%# Not IsInEditMode%>" runat="server" Text='<%# Bind("BarCode") %>'></asp:Label>
                                    ...
                                    <asp:TemplateField HeaderText="Status" SortExpression="Status">
                                        <EditItemTemplate>
                                            <asp:TextBox ID="StatusTxtbox" runat="server" Text='<%# Bind("Status") %>'></asp:TextBox>
                                        </EditItemTemplate>
                                        <ItemTemplate>
    ...
    <asp:SqlDataSource ID="SqlDataSourceTransactionPickupItems" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>" SelectCommand="SELECT Id, TransactionHeaders_id, BarCode, QRCode, Department, BoxNumber, Description, Retention, Status, CreatedBy, CreatedDate, ModifiedBy, ModifiedDate, filepath FROM PickUps WHERE (TransactionHeaders_id = @TransactionHeaders_id) AND (Status = @Status)" DeleteCommand="DELETE FROM PickUps WHERE (TransactionHeaders_id = @Id)" InsertCommand="INSERT INTO [PickUps] ([TransactionHeaders_id], [BarCode], [QRCode], [Department], [BoxNumber], [Description], [Retention], [Status], [CreatedBy], [CreatedDate], [ModifiedBy], [ModifiedDate], [filepath]) VALUES (@TransactionHeaders_id, @BarCode, @QRCode, @Department, @BoxNumber, @Description, @Retention, @Status, @CreatedBy, @CreatedDate, @ModifiedBy, @ModifiedDate, @filepath)" UpdateCommand="UPDATE PickUps SET BarCode = @BarCode, QRCode = @QRCode, Department = @Department, BoxNumber = @BoxNumber, Description = @Description, Retention = @Retention, Status = @Status WHERE (Id = @Id)">
        <DeleteParameters>
            <asp:ControlParameter ControlID="TransactionPickupsGridView" Name="Id" PropertyName="SelectedValue" Type="Int32" />
        </DeleteParameters>
        <InsertParameters>
    ...
    <asp:SqlDataSource ID="ReceiveSqlDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>" DeleteCommand="DELETE FROM [MasterList] WHERE [Id] = @Id" InsertCommand="INSERT INTO [MasterList] ([Department], [BarCode], [QRCode], [BoxNumber], [Description], [Retention], [Status], [LocationCode], [DestructionDate], [StockReceipt_id], [CompanyCode], [CreatedBy], [CreatedDate], [ModifiedBy], [ModifiedDate], [Pickup],[Retrieval],[Repickup],[Destruction]) VALUES (@Department,@BarCode,@QRCode,@BoxNumber,@Description,@Retention,@Status,@LocationCode,@DestructionDate,@StockReceipt_id,@CompanyCode,@CreatedBy,@CreatedDate,@ModifiedBy,@ModifiedDate,1,0,0,0) SET @masterID = SCOPE_IDENTITY()" SelectCommand="SELECT * FROM [MasterList]" UpdateCommand="UPDATE [MasterList] SET [Department] = @Department, [BarCode] = @BarCode, [QRCode] = @QRCode, [BoxNumber] = @BoxNumber, [Description] = @Description, [Retention] = @Retention, [Status] = @Status, [LocationCode] = @LocationCode, [DestructionDate] = @DestructionDate, [StockReceipt_id] = @StockReceipt_id, [CompanyCode] = @CompanyCode, [CreatedBy] = @CreatedBy, [CreatedDate] = @CreatedDate, [ModifiedBy] = @ModifiedBy, [ModifiedDate] = @ModifiedDate WHERE [Id] = @Id">
        <DeleteParameters>
            <asp:Parameter Name="Id" Type="Int32" />
        </DeleteParameters>
        <InsertParameters>
    ...
    <asp:SqlDataSource ID="SqlDataSourceUpdatePickupItems" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>" DeleteCommand="DELETE FROM [PickUps] WHERE [Id] = @Id" InsertCommand="INSERT INTO [PickUps] ([Status]) VALUES (@Status)" SelectCommand="SELECT [Id], [Status] FROM [PickUps]" UpdateCommand="UPDATE PickUps SET Status = @Status, ModifiedDate = @ModifiedDate, ModifiedBy = @ModifiedBy WHERE (Id = @Id)">
        <DeleteParameters>
            <asp:Parameter Name="Id" Type="Int32" />
        </DeleteParameters>
        <InsertParameters>
    ...
    <asp:SqlDataSource ID="SqlDataSourcePickupCount" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>" SelectCommand="SELECT COUNT(*) AS TotalBoxes FROM PickUps WHERE (TransactionHeaders_id = @TransactionID) AND (Status = @Status)" DataSourceMode="DataReader">
        <SelectParameters>
