...
Namespace ssiFinal
	Public Class ProcessedRetrievalTransactions
		...
		Protected Overridable Property SqlDataSourceTransactionRetrievalDetails As SqlDataSource

		Protected Overridable Property SqlDataSourceTransactionRetrievalGrid As SqlDataSource

		Protected Overridable Property SqlDataSourceTransactionRetrievalItems As SqlDataSource
		...
		Private Sub Button1_Click(ByVal sender As Object, ByVal e As EventArgs)
			...
			Try
				...
				Dim generateDeliveryReceipt As ssiFinal.GenerateDeliveryReceipt = New ssiFinal.GenerateDeliveryReceipt()
				Dim instance As PdfWriter = PdfWriter.GetInstance(document, fileStream)
				instance.PageEvent = generateDeliveryReceipt
				document.Open()
				...
				generateDeliveryReceipt = New ssiFinal.GenerateDeliveryReceipt()
				...
				Try
					...
					While enumerator1.MoveNext()
						...
						phrase1.Add(New Chunk(String.Concat("Customer: ", Me.TransactionRetrievalDetailsView.Rows(11).Cells(1).Text, "" & VbCrLf & "" & VbCrLf & ""), FontFactory.GetFont("Calibri", 10!, 1)))
						...
						phrase1.Add(New Chunk(String.Concat("Contact Number: ", Me.TransactionRetrievalDetailsView.Rows(7).Cells(1).Text, "" & VbCrLf & "" & VbCrLf & ""), FontFactory.GetFont("Calibri", 10!, 1)))
						phrase1.Add(New Chunk(String.Concat("Contact Address: ", Me.TransactionRetrievalDetailsView.Rows(3).Cells(1).Text), FontFactory.GetFont("Calibri", 10!, 1)))
				...
				MyBase.Response.AppendHeader("Content-Disposition", "attachment; filename=DeliveryReceipt.pdf")
		...
		Protected Sub ButtonSearch_Click(ByVal sender As Object, ByVal e As EventArgs)
			Try
				If (Me.DropDownListSearch.SelectedIndex = 0) Then
					Me.SqlDataSourceTransactionRetrievalItems.FilterExpression = String.Concat("BarCode LIKE '%", Me.TextBoxSearch.Text.Trim(), "%'")
				ElseIf (Me.DropDownListSearch.SelectedIndex = 1) Then
					Me.SqlDataSourceTransactionRetrievalItems.FilterExpression = String.Concat("Description LIKE '%", Me.TextBoxSearch.Text.Trim(), "%'")
				ElseIf (Me.DropDownListSearch.SelectedIndex = 2) Then
					Me.SqlDataSourceTransactionRetrievalItems.FilterExpression = String.Concat("BoxNumber = '", Me.TextBoxSearch.Text.Trim(), "'")
				ElseIf (Me.DropDownListSearch.SelectedIndex = 3) Then
					Me.SqlDataSourceTransactionRetrievalItems.FilterExpression = String.Concat("Department = '", Me.TextBoxSearch.Text.Trim(), "'")
				End If
				Me.SqlDataSourceTransactionRetrievalItems.[Select](DataSourceSelectArguments.Empty)
			Catch exception As System.Exception
				ProjectData.SetProjectError(exception)
				ProjectData.ClearProjectError()
			End Try
			...
			Try
				Try
					While sqlDataReader.Read()
						...
						htmlGenericControl.InnerHtml = Conversions.ToString(Operators.AddObject(htmlGenericControl1.InnerHtml, Operators.AddObject(Operators.AddObject(Operators.AddObject(Operators.AddObject(String.Concat("<li><a class='none' href='AdminMessage.aspx?InboxID=", sqlDataReader("ConvoId").ToString(), "'>You have a new message from "), sqlDataReader("Sender")), "<br /><strong>"), sqlDataReader("DateCreated")), "</strong></a></li>")))
						num = num + 1
		...
		Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
			...
			If (Not Information.IsNothing(MyBase.Request("ID"))) Then
				Me.SqlDataSourceTransactionRetrievalGrid.FilterExpression = String.Concat("Id = '", MyBase.Request("ID"), "'")
				Me.TransactionRetrievalGridView.SelectedIndex = 0
			End If
		End Sub

		Protected Sub RefreshButton_Click(ByVal sender As Object, ByVal e As EventArgs)
			MyBase.Response.Redirect("ProcessedRetrievalTransactions")
			Try
		...
		Public Sub renderErrMsgs(ByVal thisErrMsg As String)
			' 
			' Current member / type: System.Void ssiFinal.ProcessedRetrievalTransactions::renderErrMsgs(System.String)
		...
		Protected Sub SearchButton_Click(ByVal sender As Object, ByVal e As EventArgs)
			Try
				If (Me.SearchDropDownList.SelectedIndex = 0) Then
					Me.SqlDataSourceTransactionRetrievalGrid.FilterExpression = String.Concat("DepartmentCode LIKE '%", Me.SearchTextbox.Text.Trim(), "%'")
				ElseIf (Me.SearchDropDownList.SelectedIndex = 1) Then
					Me.SqlDataSourceTransactionRetrievalGrid.FilterExpression = String.Concat("ContactName LIKE '%", Me.SearchTextbox.Text.Trim(), "%'")
				ElseIf (Me.SearchDropDownList.SelectedIndex = 2) Then
					Me.SqlDataSourceTransactionRetrievalGrid.FilterExpression = String.Concat("Id = '", Me.SearchTextbox.Text.Trim(), "'")
				ElseIf (Me.SearchDropDownList.SelectedIndex = 3) Then
					Me.SqlDataSourceTransactionRetrievalGrid.FilterExpression = String.Concat("RequestDate = '", Me.SearchTextbox.Text.Trim(), "'")
				End If
				Me.SqlDataSourceTransactionRetrievalGrid.[Select](DataSourceSelectArguments.Empty)
			Catch exception As System.Exception
				ProjectData.SetProjectError(exception)
				ProjectData.ClearProjectError()
			End Try
