...
Namespace ssiFinal
	Public Class destruction
        ...
        Private _AddToDestructionButton As Button
        ...
        Protected Overridable Property AddToDestructionButton As Button
            Get
                Return Me._AddToDestructionButton
            End Get
            <MethodImpl(MethodImplOptions.Synchronized)>
            Set(ByVal value As System.Web.UI.WebControls.Button)
                Dim eventHandler As System.EventHandler = New System.EventHandler(AddressOf Me.AddToDestructionButton_Click)
                Dim button As System.Web.UI.WebControls.Button = Me._AddToDestructionButton
                If (button IsNot Nothing) Then
                    RemoveHandler button.Click, eventHandler
                End If
                Me._AddToDestructionButton = value
                button = Me._AddToDestructionButton
                If (button IsNot Nothing) Then
		...
		Protected Overridable Property SqlDataSourceInsertDestructionItems As SqlDataSource
		...
		Protected Overridable Property TextBoxContactAddress As TextBox

		Protected Overridable Property TextBoxDestructionDate As TextBox
		...
		Protected Sub AddToDestructionButton_Click(ByVal sender As Object, ByVal e As EventArgs)
			...
			Try
				Try
					...
					While enumerator.MoveNext()
						...
						If (Not TryCast(current.Cells(0).FindControl("AddTodestructionCheckBox"), CheckBox).Checked) Then
							Continue While
						End If
						...
						dataRow("QRCode") = HttpUtility.HtmlDecode(current.Cells(5).Text)
						dataRow("Box Number") = HttpUtility.HtmlDecode(current.Cells(4).Text)
		...
		Protected Sub AddTodestructionCheckBox_CheckedChanged(ByVal sender As Object, ByVal e As EventArgs)
			Dim enumerator As IEnumerator = Nothing
			Try
				enumerator = Me.GridViewAvailableBoxes.Rows.GetEnumerator()
				While enumerator.MoveNext()
					Dim current As GridViewRow = DirectCast(enumerator.Current, GridViewRow)
					Dim checkBox As System.Web.UI.WebControls.CheckBox = TryCast(current.FindControl("AddTodestructionCheckBox"), System.Web.UI.WebControls.CheckBox)
		...
		Protected Sub chkboxSelectAll_CheckedChanged(ByVal sender As Object, ByVal e As EventArgs)
			...
			Try
				...
				While enumerator.MoveNext()
					Dim current As GridViewRow = DirectCast(enumerator.Current, GridViewRow)
					Dim checkBox1 As System.Web.UI.WebControls.CheckBox = DirectCast(current.FindControl("AddTodestructionCheckBox"), System.Web.UI.WebControls.CheckBox)
					Dim num As Integer = Convert.ToInt32(RuntimeHelpers.GetObjectValue(Me.GridViewAvailableBoxes.DataKeys(current.RowIndex)("Id")))
		...
		Public Sub EmailNotification()
			If (MyBase.User.IsInRole("clientSupervisor")) Then
				Using mailMessage As System.Net.Mail.MailMessage = New System.Net.Mail.MailMessage("ssi_mailsender@ssistorage.com", "ssi_mail@ssistorage.com")
					mailMessage.Subject = "Destruction Request"
					...
					Dim alternateView As System.Net.Mail.AlternateView = System.Net.Mail.AlternateView.CreateAlternateViewFromString(String.Concat(New String() { "<div style=""border-top:3px solid #FF0000"">&nbsp;</div><table><tr><td><img src=""cid:image1"" height=""75x"" width=""75px""/></td><td><b>SSI Storage Solutions Inc.</b></td></tr></table><br/><br/>You have new destruction request from ", Me.contactNameTextBox.Text, " of ", Me.CompanyNameTextBox.Text, ".<br/><br/><div style=""border-top:3px solid #FF0000"">&nbsp;</div>" }), Nothing, "text/html")
		...
		Private Sub GridViewAvailableBoxes_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs)
			...
			If (row.RowType = DataControlRowType.DataRow) Then
				Dim checkBox As System.Web.UI.WebControls.CheckBox = TryCast(row.FindControl("AddTodestructionCheckBox"), System.Web.UI.WebControls.CheckBox)
		...
		Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
			...
			If (Not Me.Page.IsPostBack) Then
				Try
					...
					Me.TextBoxContactAddress.Text = Conversions.ToString(Me.Session("Address"))
		...
		Public Sub renderErrMsgs(ByVal thisErrMsg As String)
			' 
			' Current member / type: System.Void ssiFinal.destruction::renderErrMsgs(System.String)
			...
			Try
				...
				Try
					...
					While enumerator.MoveNext()
						...
						Me.SqlDataSourceInsertDestructionItems.InsertParameters("MasterList_Id").DefaultValue = current.Cells(1).Text
						Me.SqlDataSourceInsertDestructionItems.InsertParameters("BarCode").DefaultValue = HttpUtility.HtmlDecode(current.Cells(2).Text)
						Me.SqlDataSourceInsertDestructionItems.InsertParameters("QRCode").DefaultValue = HttpUtility.HtmlDecode(current.Cells(3).Text)
						Me.SqlDataSourceInsertDestructionItems.InsertParameters("BoxNumber").DefaultValue = HttpUtility.HtmlDecode(current.Cells(4).Text)
						Me.SqlDataSourceInsertDestructionItems.InsertParameters("Description").DefaultValue = HttpUtility.HtmlDecode(current.Cells(5).Text)
						Me.SqlDataSourceInsertDestructionItems.InsertParameters("Department").DefaultValue = HttpUtility.HtmlDecode(current.Cells(6).Text)
						Me.SqlDataSourceInsertDestructionItems.Insert()
					End While
				Finally
					If (TypeOf enumerator Is IDisposable) Then
						TryCast(enumerator, IDisposable).Dispose()
		...
		Protected Sub SubmitButton_Click(ByVal sender As Object, ByVal e As EventArgs)
			Try
				If (Me.GridView1.Rows.Count <= 0) Then
					Me.renderErrMsgs("There are no items in Destruction list.")
				ElseIf (Microsoft.VisualBasic.CompilerServices.Operators.CompareString(Me.TextBoxDestructionDate.Text, "", False) = 0) Then
					Me.renderErrMsgs("Destruction date is required.")
				ElseIf (Microsoft.VisualBasic.CompilerServices.Operators.CompareString(Me.TextBoxContactAddress.Text, "", False) = 0) Then
					Me.renderErrMsgs("Contact address is required.")
				...
				ElseIf (Microsoft.VisualBasic.CompilerServices.Operators.CompareString(Me.contactNumberTextBox.Text, "", False) <> 0) Then
					Me.SqlDataSourceTransHeader.InsertParameters("ContactName").DefaultValue = Me.contactNameTextBox.Text
					Me.SqlDataSourceTransHeader.InsertParameters("username").DefaultValue = Conversions.ToString(Me.Session("clientUserName"))
					Me.SqlDataSourceTransHeader.InsertParameters("Address").DefaultValue = Me.TextBoxContactAddress.Text
					Me.SqlDataSourceTransHeader.InsertParameters("PickUpDate").DefaultValue = Me.TextBoxDestructionDate.Text
					Me.SqlDataSourceTransHeader.InsertParameters("RequestDate").DefaultValue = Me.TextBoxRequestDate.Text
					Me.SqlDataSourceTransHeader.InsertParameters("DepartmentCode").DefaultValue = Conversions.ToString(Me.Session("DepartmentCode"))
					Me.SqlDataSourceTransHeader.InsertParameters("ContactNumber").DefaultValue = Me.contactNumberTextBox.Text
					If (MyBase.User.IsInRole("ssiClient")) Then
						Me.SqlDataSourceTransHeader.InsertParameters("Status").DefaultValue = "FORAPPROVAL"
					ElseIf (MyBase.User.IsInRole("clientSupervisor")) Then
						Me.SqlDataSourceTransHeader.InsertParameters("Status").DefaultValue = "NEW"
					End If
					Me.SqlDataSourceTransHeader.InsertParameters("Type").DefaultValue = "DESTRUCTION"
					Me.SqlDataSourceTransHeader.InsertParameters("CompanyCode").DefaultValue = Conversions.ToString(Me.Session("CompanyCode"))
					...
					MyBase.Response.Redirect("SuccessPage?SuccessMessage=Your request has been successfully submitted.&Type=Destruction")
				Else
